...
function [xopt, uopt] = lf_fmincon_planner(x0, planT, planDt)
...
function [c,ceq] = constraintFun(z)
    ...
    for t = 0:T-1
        ...
        ceq(nx*t+1:nx*t+nx) = xt1 - (xt + dt*lf_dyn(xt,ut)); 
    end
end
...
function xi = segmentInterp(x0, xf, t)
    global T
    % THREE SEGMENTS
    xm1 = [x0(1) + 3; (xf(2) - x0(2))/3 + x0(2)];
    xm2 = [x0(1) + 3; 2*(xf(2) - x0(2))/3 + x0(2)];
    T1 = T/3;
    T2 = 2*T/3;
    if t < T1
        xi = x0 + (t / T1) * (xm1 - x0);
...
%% Plotting
function plotTraj(xopt, init, x0)
    ...
    plot(x0(1), x0(2), 'bo', 'MarkerSize', 10);
    ...
    p1 = plot(traj(1,:), traj(2,:), 'r','MarkerSize', 5, 'MarkerEdgeColor', color, 'MarkerFaceColor', color);
    p1.Color(4) = 0.9;
